...
export class OrdersService {
  ...
  async create(items: { productId: number; quantity: number }[], source: 'ecom' | 'pos' = 'ecom') {
    ...
    try {
      ...
      for (const it of items) {
        ...
        if (prod.stock < it.quantity) throw new BadRequestException(`Insufficient stock for ${prod.name}`);
        total += prod.price * it.quantity;
      }

      const order = await Order.create({ total, source } as any, { transaction: t });

      for (const it of items) {
        await OrderItem.create({ orderId: order.id, productId: it.productId, quantity: it.quantity } as any, { transaction: t });
        const prod = await Product.findByPk(it.productId, { transaction: t });
        if (!prod) throw new BadRequestException(`Product ${it.productId} not found`);
